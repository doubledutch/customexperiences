...
class HomeView extends PureComponent {
  ...
  componentDidMount() {
    ...
    client.getCurrentUser().then(currentUser => {
      ...
      this.loadLocalTemplates().then(localTemplates => {
        this.signin
          .then(() => {
  ...
  findConfig = templates => {
    ...
    let currentTemplate = []
    if (currentI !== null) {
      currentTemplate = templates[currentI]
    }
  ...
  leadStorageKey() {
    return `@DD:customExperiences_${this.state.currentEvent.id}_${this.state.currentUser.id}`
  }
}
