...
class App extends PureComponent {
  constructor(props) {
    ...
    client
      .cmsRequest('GET', '/api/Principal')
      .then(user => {
        const shouldShow = !!user.Permissions.find(
          p => p.Verb === 'Update' && p.Resource === 'DoubleDutchInternalSetting',
        )
  ...
  showCell = object => {
    ...
    const newCell = JSON.parse(JSON.stringify(origData)) // Quick and dirty deep copy
    if (newCell.type === 'Landing Page Cell') {
      newCell.bold = object.boldBool
    }
  ...
  handleEdit = event => {
    ...
    const newCell = Object.assign({}, this.state.items[item])
    ...
    this.signin.then(user => {
      client.getCurrentEvent().then(evt => {
        ...
        templateRef.on('child_added', data => {
          const template = this.saveHour(data.val())
        ...
        templateRef.on('child_changed', data => {
          ...
          const newArray = this.state.templates
          const i = newArray.findIndex(item => item.key === name)
          newArray.splice(i, 1)
          this.setState({ templates: [...this.state.templates, { ...template, key: data.key }] })
        })
        templateRef.on('child_removed', data => {
          this.setState({
            templates: this.state.templates.filter(x => x.key !== data.key),
            items: [],
          })
        })
      })
  ...
  submitEventData = (origDate, currentEdit) => {
    const publishTime = [
      { publishDate: origDate.getTime(), requireScroll: this.state.requireScroll },
    ]
    const items = this.state.items
    let title = this.state.value
    if (title) {
      title = this.state.value.toLowerCase()
    }
    ...
    if (items.length) {
      ...
      const newItems = publishTime.concat(items)
  ...
  deleteLastSpeaker = () => {
    const newCell = Object.assign({}, this.state.newCell)
  ...
  loadTemplate = event => {
    ...
    const item = this.state.templates.find(item => item.key === title)
    for (const i in item) {
      if (i !== 'key') {
  ...
  render() {
    ...
    if (shouldShow === false)
      return (
        <div>
          Please contact your customer experience manager about configuring custom experiences.
        </div>
      )
      ...
      <div className="App">
        ...
        <div className="container">
          ...
          <div className="submitBox">
            <form className="dropdownMenu">
              ...
              >
                <option className="dropdownTitle" value="">
                  {'\xa0\xa0'}View Templates
                </option>
                {allTemplates.map((task, i) => {
                  const title = task.key
                  return (
                    <option className="dropdownTitle" key={i} value={task.key}>
                      {`\xa0\xa0${title}`}
                    </option>
                  )
                })}
        ...
        <div className="buttonsContainer">
          <button
            ...
            disabled={!this.state.items.length || this.state.showFormBool}
            value="false"
          >
            Publish to App
          </button>
          <button
            className="modalButton"
            style={{ marginRight: 40, fontSize: 18, backgroundColor: 'red' }}
            disabled={!this.state.value || this.state.showFormBool}
        ...
        <div className="expoContainer">
          ...
          <p>
            Copy & Paste the data below into the "dataInput" object to build your test app in Expo
          </p>
